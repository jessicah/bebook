...
more complex settings, the {cpp:func}`~get::driver` function is available
to retrieve all your settings in a hierarchical tree.
...
- Call {cpp:func}`~load::driver` to load the settings data.

- Use {cpp:func}`~get::driver` or {cpp:func}`~get::driver` and
{cpp:func}`~get::driver` to read the settings.

- Call {cpp:func}`~unload::driver` when you're done.
...
following form in the settings file:name [value]* [{ [parameter]* }]
['n',',']Where [ … ] indicates an optional part, and [ … ]* indicates an
optional repeated part.
...
For this settings file, {cpp:func}`~get::driver` will return a pointer to
the following tree:
...
To load the driver's settings, you need to call {cpp:func}`~load::driver`.
...
The handle is then used when calling the other driver settings functions,
...
- {cpp:func}`~get::driver` returns a boolean parameter's value.

- {cpp:func}`~get::driver` returns a string parameter's value.

- {cpp:func}`~get::driver` returns all the settings at once, encapsulated in
...
If there's no settings file, {cpp:func}`~load::driver` will return
{cpp:enum}`NULL`. In this case, {cpp:func}`~get::driver` will return
{cpp:enum}`false` (the value we're passing as the {hparam}`unknownValue`
argument).
...
line is used as the value. If no value is specified, {cpp:enum}`true` is
returned (the value of the {hparam}`noArgValue` argument to
{cpp:func}`~get::driver`). Otherwise the following is done:

- If the value is "1", "true", "yes", "on", "enable", or "enabled",
{cpp:enum}`true` is returned.

- If the value is "0", "false", "no", "off", "disable", or "disabled",
{cpp:enum}`false` is returned.
...
{cpp:func}`~get::driver` function. The only difference is that the string
returned will be {cpp:enum}`NULL` if the parameter is missing, or the file
doesn't exist.
...
tree using the {cpp:func}`~get::driver` function.
...
is specified by the handle, as returned by {cpp:func}`~load::driver`. The
parameter's name is given by {hparam}`keyName`. If the parameter isn't
...
If the {hparam}`handle` is {cpp:enum}`NULL`, {hparam}`unknownValue` is
returned.
...
{cpp:func}`~load::driver`. The parameter's name is given by
...
The special {hparam}`keyName` value {cpp:enum}`B_SAFEMODE_SAFE_MODE` can
be used if you want to find out whether or not BeOS was booted in safe
mode; the value will be {cpp:enum}`true` if BeOS is running in safe mode,
...
settings, pass {cpp:enum}`B_SAFEMODE_DRIVER_SETTINGS` Returns
{cpp:enum}`NULL` if no settings are available for the driver.
