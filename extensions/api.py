...
class ASTIdentifier(ASTBase):
    ...
    def describe_signature(self, signode: TextElement, mode: str, env: "BuildEnvironment",
        ...
        if mode == 'markType':
            ...
            pnode = addnodes.pending_xref('', refdomain='api',
                                          reftype='identifier',
                                          reftarget=targetText, modname=None,
                                          classname=None)
            pnode['api:parent_key'] = symbol.get_lookup_key()
            pnode += node
            signode += pnode
...
class Symbol:
    ...
    def __init__(self, parent: Optional["Symbol"],
        ...
        #
        #     .. api:class:: template <typename T> class A
        #
        # and
        #
        #     .. api:function:: template <typename T> int A<T>::foo()
...
class CPPObject(ObjectDescription[ASTDeclaration]):
    ...
    def run(self) -> List[Node]:
        ...
        if 'api:parent_symbol' not in env.temp_data:
            root = env.domaindata['cpp']['root_symbol']
            env.temp_data['api:parent_symbol'] = root
            env.ref_context['api:parent_key'] = root.get_lookup_key()

        # The lookup keys assume that no nested scopes exists inside overloaded functions.
        # (see also #5191)
        # Example:
        # .. api:function:: void f(int)
        # .. api:function:: void f(double)
        #
        #    .. api:function:: void g()
        #
        #       :api:any:`boom`
        ...
        parentSymbol = env.temp_data['api:parent_symbol']
        parentDecl = parentSymbol.declaration
        if parentDecl is not None and parentDecl.objectType == 'function':
            ...
            env.temp_data['api:last_symbol'] = symbol
            return []
        # When multiple declarations are made in the same directive
        # they need to know about each other to provide symbol lookup for function parameters.
        # We use last_symbol to store the latest added declaration in a directive.
        env.temp_data['api:last_symbol'] = None
        return super().run()

    def handle_signature(self, sig: str, signode: desc_signature) -> ASTDeclaration:
        parentSymbol: Symbol = self.env.temp_data['api:parent_symbol']
        ...
        try:
            ...
            name = _make_phony_error_name()
            symbol = parentSymbol.add_name(name)
            self.env.temp_data['api:last_symbol'] = symbol
        ...
        try:
            ...
            symbol.siblingAbove = self.env.temp_data['api:last_symbol']
            ...
            self.env.temp_data['api:last_symbol'] = e.symbol
            msg = __("Duplicate C++ declaration, also defined at %s:%s.\n"
                     "Declaration is '.. api:%s:: %s'.")
    ...
    def before_content(self) -> None:
        lastSymbol: Symbol = self.env.temp_data['api:last_symbol']
        assert lastSymbol
        self.oldParentSymbol = self.env.temp_data['api:parent_symbol']
        self.oldParentKey: LookupKey = self.env.ref_context['api:parent_key']
        self.env.temp_data['api:parent_symbol'] = lastSymbol
        self.env.ref_context['api:parent_key'] = lastSymbol.get_lookup_key()

    def after_content(self) -> None:
        self.env.temp_data['api:parent_symbol'] = self.oldParentSymbol
        self.env.ref_context['api:parent_key'] = self.oldParentKey
...
class CPPNamespaceObject(SphinxDirective):
    ...
    def run(self) -> List[Node]:
        ...
        if self.arguments[0].strip() in ('NULL', '0', 'nullptr'):
            ...
            stack = [symbol]
        self.env.temp_data['api:parent_symbol'] = symbol
        self.env.temp_data['api:namespace_stack'] = stack
        self.env.ref_context['api:parent_key'] = symbol.get_lookup_key()
        return []
...
class CPPNamespacePushObject(SphinxDirective):
    ...
    def run(self) -> List[Node]:
        ...
        try:
            ...
            name = _make_phony_error_name()
            ast = ASTNamespace(name, None)
        oldParent = self.env.temp_data.get('api:parent_symbol', None)
        if not oldParent:
            oldParent = self.env.domaindata['cpp']['root_symbol']
        symbol = oldParent.add_name(ast.nestedName, ast.templatePrefix)
        stack = self.env.temp_data.get('api:namespace_stack', [])
        stack.append(symbol)
        self.env.temp_data['api:parent_symbol'] = symbol
        self.env.temp_data['api:namespace_stack'] = stack
        self.env.ref_context['api:parent_key'] = symbol.get_lookup_key()
...
class CPPNamespacePopObject(SphinxDirective):
    ...
    def run(self) -> List[Node]:
        stack = self.env.temp_data.get('api:namespace_stack', None)
    ...
    def __init__(self, sig: str, aliasOptions: dict,
        ...
        if env is not None:
            if 'api:parent_symbol' not in env.temp_data:
                ...
                env.temp_data['api:parent_symbol'] = root
                env.ref_context['api:parent_key'] = root.get_lookup_key()
            self.parentKey = env.ref_context['api:parent_key']
        else:
...
class CPPExprRole(SphinxRole):
    ...
    def run(self) -> Tuple[List[Node], List[system_message]]:
        ...
        parentSymbol = self.env.temp_data.get('api:parent_symbol', None)
...
class CPPDomain(Domain):
    ...
    directives = {
		...
		'functions': CPPMultipleFunctionsObject,
        # scope control
    ...
    def _resolve_xref_inner(self, env: BuildEnvironment, fromdocname: str, builder: Builder,
        ...
        try:
            ...
            logger.warning('Unparseable C++ cross-reference: %r\n%s', t, ex,
                           location=node)
            return None, None
        parentKey: LookupKey = node.get("api:parent_key", None)
        rootSymbol = self.data['root_symbol']
        if parentKey:
            parentSymbol: Symbol = rootSymbol.direct_lookup(parentKey)
            if not parentSymbol:
        ...
        def checkType() -> bool:
            ...
            raise AssertionError()
        if not checkType():
            logger.warning("api:%s targets a %s (%s).",
    ...
    def resolve_any_xref(self, env: BuildEnvironment, fromdocname: str, builder: Builder,
        ...
        if retnode:
            if objtype == 'templateParam':
                return [('api:templateParam', retnode)]
            else:
                return [('api:' + self.role_for_objtype(objtype), retnode)]
        return []
