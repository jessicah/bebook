...
The {hparam}`width` returned by this function is the width of the slider
...
{hmethod}`HashMarkCount()` returns the number of hash marks currently set
up.
::::
...
{hmethod}`SetOrientation()` sets the slider's orientation and
