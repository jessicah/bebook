...
{hmethod}`GetMode()` copies the current {htype}`display_mode` into
{hparam}`mode`.
...
-
	...
	- are:
		...
		{cpp:enumerator}`B_BLANK_PEDESTAL`
...
In the current BeOS release, this function always returns
{cpp:enumerator}`B_ERROR`.
::::
