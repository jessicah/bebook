...
choices can be the default settings when {cpp:func}`~BPrintJob::ConfigPage`
is called again. This is good behavior for an application to follow, and is
highly recommended.
...
in the previous "{cpp:func}`~BPrintJob::Overview`" section. The idea is the
...
document is next printed. By calling {cpp:func}`~BPrintJob::Settings`, you
can get the job configuration the user set up;
...
{cpp:func}`~BPrintJob::CommitJob` is called to commit them to the printer.
...
{cpp:func}`~BPrintJob::CanContinue` returns false, indicating that the job
has been canceled:
...
views have drawn, {cpp:func}`~BPrintJob::SpoolPage` spools the data to the
file that will eventually be committed to the printer.
...
measures a sheet of paper and {cpp:func}`~BPrintJob::PrintableRect` returns
the printable rectangle, as illustrated in this diagram.
...
rectangle as {cpp:func}`~BPrintJob::PrintableRect` would report them and as
