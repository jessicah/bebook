...
bytes of data. Then call {cpp:func}`~BBufferProducer::SendBuffer`. You can
then immediately reuse the buffer.
...
{cpp:enum}`B_SMALL_BUFFER` flag set, you must recycle the buffer in your
{cpp:func}`~BBufferConsumer::BufferReceived` function.
:::
